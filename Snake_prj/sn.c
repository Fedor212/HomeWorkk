...
enum {MAX_TAIL_SIZE=100, START_TAIL_SIZE=4, MAX_FOOD_SIZE=20, FOOD_EXPIRE_SECONDS=10,SEED_NUMBER=3};
...
//Код врезания головы в хвост
int if_catch_food(struct snake_t * snakee)
{
    int crushed = 1;
    for (int i = 0; i < START_TAIL_SIZE; i++)
    {
        if ((snakee->tail[i].x == snakee->x) && (snakee->tail[i].y == snakee->y))
        {
